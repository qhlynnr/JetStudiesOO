...
int PlotFineBinning(){
    ...
    string MCFile = "/home/xirong/JetStudiesOO/LynnsCode/RootFiles/0414HalfHalfTest/Response_MC_2.6MEvts_MC_10files_FineBin_Debug_20260417.root";
    string outfolder = "/home/xirong/JetStudiesOO/LynnsCode/Plots/Response/0416Unfolding_HalfhalftestDebug/FineBinningDebug/";
    string outFileTag = "5050_FineBin";
    ...
    TH1F* hTestGenPt = (TH1F*)MCFilef->Get("hGenPt2");
    TH1F* hTestRecoPt = (TH1F*)MCFilef->Get("hRecoPt2");
    ...
    vector<TH1*> AllHist = {hGenPt, hGenNoWeightPt, hRecoNoWeightPt, hRecoPt, hFakePt, hMatchedRecoPt, hMatchedGenPt, hTestGenPt, hTestRecoPt};
// Printouts
    for (int i = 1; i<= hGenNoWeightPt->GetNbinsX(); i++) {
      cout << "Bin" << i << "Gen pt: " << hGenNoWeightPt->GetBinContent(i) << " | Reco pt: " << hRecoNoWeightPt->GetBinContent(i) << endl;
    }
    for (int i = 1; i<= hGenPt->GetNbinsX(); i++) {
      cout << "Bin" << i << "Gen pt: " << hGenPt->GetBinContent(i) << " | Reco pt: " << hRecoPt->GetBinContent(i) << endl;
    }
    for (int i = 1; i<= hGenPt->GetNbinsX(); i++) {
      cout << "Bin" << i << "Error on Gen pt: " << hGenPt->GetBinError(i) << " | Error on Reco pt: " << hRecoPt->GetBinError(i) << endl;
    }
    for (auto h : AllHist) h->Scale(1.0 / h->GetSumOfWeights());

    vector<TH1*> hUnfoldedIters;
    vector<string> iterLabels;
// ############ Cut info and general info ############
    TNamed* cutInfo= (TNamed*)MCFilef->Get("Cuts");
    TNamed* generalInfo = (TNamed*)MCFilef->Get("GeneralInfo");
  ...
  // ################### Start Gen Reco Test ###################
    ...
    DrawAndSave(hRatioIters,
                ...
                30, -999, 0.5, 1.5, false, false, captionInfo);
