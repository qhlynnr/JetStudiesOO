...
public:
    ...
    bool JetSelection_pp(Float_t jteta, Float_t jtphi, Float_t CEF, Float_t NEF, Float_t MUF,
                        Float_t NHF, Float_t CHF, Float_t CHM){
        bool KeepJet=false;
        bool KeepJet_map=false;
        bool KeepJet_id=false;
        if(TMath::Abs(jteta)<=2.6){
            if((NEF<0.9)&&(MUF<0.8)&&(CEF<0.8)&&(NHF<0.9)&&(CHF>0.01)&&(CHM>0)){
                KeepJet_id=true;
            }
        }
        if((TMath::Abs(jteta)>2.6)&&(TMath::Abs(jteta)<=2.7)){
            if((NHF<0.90)&&(NEF<0.99)&&(MUF<0.8)&&(CEF<0.8)){
                KeepJet_id=true;
            }
        }
        if((TMath::Abs(jteta)>2.7)&&(TMath::Abs(jteta)<=3.0)){
            if((NHF<0.9999)){
                KeepJet_id=true;
            }
        }
        if((TMath::Abs(jteta)>3)&&(TMath::Abs(jteta)<=5.0)){
            if((NEF<0.9)){
                KeepJet_id=true;
            }
        }
