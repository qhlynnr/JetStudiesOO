...
// Derive a short, histogram-safe level name from a JEC text file path, e.g.
//   ".../Prompt24HIpp_V1_MC_L2Relative_AK4PF.txt"  ->  "L2Relative"
//   ".../Summer24Prompt24_RunGnib2_V4_DATA_L2L3Residual_AK4PFPuppi.txt" -> "L2L3Residual"
// Falls back to "step<N>" if no recognizable level token is present, so the
// histogram names stay unique and valid whatever files get passed in.
static std::string JECLevelName(const std::string& path, int index)
{
    std::string base = gSystem->BaseName(path.c_str());
    // strip the ".txt" extension if present
    size_t dot = base.rfind('.');
    if (dot != std::string::npos) base = base.substr(0, dot);

    // Known JEC level tokens, longest first so L2L3Residual wins over L2Relative/L3Absolute
    const std::vector<std::string> levels = {
        "L1L2L3Residual", "L2L3Residual", "L1FastJet", "L1Offset", "L1RC",
        "L2Relative", "L3Absolute", "L2Residual", "L3Residual", "L5Flavor", "L7Parton"
    };
    for (const auto& lvl : levels) {
        if (base.find(lvl) != std::string::npos) return lvl;
    }
    return Form("step%d", index + 1);
}
                                        ...
                                        
{
 ...
 //   int L1_SingleJet28_BptxAND;
   ...
   // double ptBins[] = {80, 140, 200, 300, 400, 500};
    ...
    // Sequential-JEC histograms: one pT spectrum after each correction level is
    // applied on top of the previous ones, named hcorrectedpt_step<N>_<Level>
    // (e.g. hcorrectedpt_step1_L2Relative, hcorrectedpt_step2_L2L3Residual).
    // The last one is by construction identical to hcorrectedpt.
    std::vector<std::string> jecLevelNames;
    std::vector<TH1F*> hptAfterLevel;
    for (size_t s = 0; s < JECtxtvector.size(); ++s){
        std::string lvl = JECLevelName(JECtxtvector[s], (int)s);
        jecLevelNames.push_back(lvl);
        std::string hname  = Form("hcorrectedpt_step%d_%s", (int)s + 1, lvl.c_str());
        std::string htitle = Form("Jet pT after JEC step %d (%s); Jet pT (GeV/c); Number of Jets",
                                  (int)s + 1, lvl.c_str());
        hptAfterLevel.push_back(new TH1F(hname.c_str(), htitle.c_str(), nptBins, ptBins));
        cout << "JEC step " << (s + 1) << ": " << lvl << "  ->  " << hname << endl;
    }
   ...
   // JetAnalyserTree->SetBranchAddress("hltanalysis.L1_SingleJet28_BptxAND",&L1_SingleJet28_BptxAND);
    ...
    // One corrector per level, so the running pT can be recorded after each
    // text file is applied. Applying them in this order, feeding each output
    // into the next, reproduces JEC.GetCorrectedPT() exactly at the last step.
    std::vector<SingleJetCorrector*> JECsteps;
    for (size_t s = 0; s < Files.size(); ++s){
        JECsteps.push_back(new SingleJetCorrector(Files[s]));
    }
    std::vector<double> ptAfterLevel(JECsteps.size(), -1.0);
    ...
    for (Long64_t entrynum = 0; entrynum < nEntries; entrynum++){
        ...
        for (int j = 0; j < nref; j++){
            ...
            // Same chain again, but keeping the running pT after each level.
            // Mirrors JetCorrector::GetCorrectedPT(): each level takes the
            // previous level's output pT, and a negative value aborts the chain.
            ptAfterLevel.assign(JECsteps.size(), -1.0);
            {
                double ptRunning = rawpt[j];
                for (size_t s = 0; s < JECsteps.size(); ++s){
                    JECsteps[s]->SetJetPT(ptRunning);
                    JECsteps[s]->SetJetEta(jteta[j]);
                    JECsteps[s]->SetJetPhi(jtphi[j]);
                    ptRunning = JECsteps[s]->GetCorrectedPT();
                    if (ptRunning < 0) break;
                    ptAfterLevel[s] = ptRunning;
                }
            }
            ...
            float response = recojetpt / genjetpt;
            if (JetPtCutBool == 1){
                if (genjetpt < jtptminCut) continue;
                if (genjetpt > jtptmaxCut) continue;
                if (jet_pt_corr < jtptminCut) continue;
                if (jet_pt_corr > jtptmaxCut) continue;
            ...
            // pT after each successive correction level, same jet sample as hcorrectedpt
            for (size_t s = 0; s < hptAfterLevel.size(); ++s){
                if (ptAfterLevel[s] < 0) continue;   // level failed for this jet
                hptAfterLevel[s]->Fill(ptAfterLevel[s], totalWeight);
            }
    ...
    string outFileName = Form("../../../RootFiles/%s/%s_%sEvts_%s_%s.root",
    ...
    // Order in which the JEC text files were applied, so plotting macros can
    // find the hcorrectedpt_step<N>_<Level> histograms without hardcoding names
    {
        std::string jecOrder;
        for (size_t s = 0; s < jecLevelNames.size(); ++s){
            if (s) jecOrder += ",";
            jecOrder += jecLevelNames[s];
        }
        generalInfoString += Form("JECLevels: %s;", jecOrder.c_str());
    }
    ...
    hrawpt->Write();
    hcorrectedpt->Write();
    for (auto h : hptAfterLevel) h->Write();
    hjteta->Write();
    hjtphi->Write();
    hjtrg->Write();
    hjtzg->Write();
    ...
    for (auto h : hptAfterLevel) delete h;
    for (auto p : JECsteps) delete p;
