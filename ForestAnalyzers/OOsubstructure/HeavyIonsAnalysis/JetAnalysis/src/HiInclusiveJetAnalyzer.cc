...
HiInclusiveJetAnalyzer::HiInclusiveJetAnalyzer(const edm::ParameterSet& iConfig) {
  ...
  doIterativeDeclustering_ = iConfig.getUntrackedParameter<bool>("doIterativeDeclustering", false); 
...
void HiInclusiveJetAnalyzer::beginJob() {
  ...
  if(doIterativeDeclustering_){
    t->Branch("jtzg",jets_.jtzg,"jtzg[nref]/F");
    t->Branch("jtrg",jets_.jtrg,"jtrg[nref]/F");
    t->Branch("jtkt",jets_.jtkt,"jtkt[nref]/F");
  ...
  if (isMC_) {
    ...
    if(doIterativeDeclustering_){
      t->Branch("refzg",jets_.refzg,"refzg[nref]/F");
      t->Branch("refrg",jets_.refrg,"refrg[nref]/F");
      t->Branch("refkt",jets_.refkt,"refkt[nref]/F");
    ...
    if (fillGenJets_) {
      ...
      if(doIterativeDeclustering_){
        t->Branch("genzg",jets_.genzg,"genzg[ngen]/F");
        t->Branch("genrg",jets_.genrg,"genrg[ngen]/F");
        t->Branch("genkt",jets_.genkt,"genkt[ngen]/F");
        t->Branch("genangu",jets_.genangu,"genangu[ngen]/F");
      }
...
void HiInclusiveJetAnalyzer::analyze(const Event& iEvent, const EventSetup& iSetup) {
  ...
  */
  ...
  for (unsigned int j = 0; j < jets->size(); ++j) {
    ...
    JEC.SetJetPT(jet.correctedJet("Uncorrected").pt());
    JEC.SetJetEta(jet.eta());
    JEC.SetJetPhi(jet.phi());  

    //auto pt = useRawPt_ ? jet.correctedJet("Uncorrected").pt() : jet.pt();
    auto pt = useRawPt_ ? jet.correctedJet("Uncorrected").pt() : JEC.GetCorrectedPT();
    if (pt < jetPtMin_)
      continue;
    ...
    */
    ...
    jets_.jtpt[jets_.nref] =  JEC.GetCorrectedPT(); //jet.pt();
    ...
    if( doIterativeDeclustering_){
      jets_.jtzg[jets_.nref]  = 0;
      jets_.jtrg[jets_.nref]  = 0;
      jets_.jtkt[jets_.nref]  = 0;
    ...
    if (isMC_) {
      ...
      if (genjet) {
        ...
        if( doIterativeDeclustering_){
          jets_.refzg[jets_.nref]  = 0;
          jets_.refrg[jets_.nref]  = 0;
          jets_.refkt[jets_.nref]  = 0;
          jets_.refangu[jets_.nref]= 0;
          
          fastjet::PseudoJet *sub1Ref = new fastjet::PseudoJet();
          fastjet::PseudoJet *sub2Ref = new fastjet::PseudoJet();
        
          IterativeDeclusteringRef(*genjet,sub1Ref, sub2Ref);
        }
        ...
        if( doIterativeDeclustering_){
          jets_.refzg[jets_.nref]  = -999.;
          jets_.refrg[jets_.nref]  = -999.;
          jets_.refkt[jets_.nref]  = -999.;
          jets_.refangu[jets_.nref]= -999.;
        }
      }
  ...
  if (isMC_) {
    ...
    for (unsigned int igen = 0; igen < genjets->size(); ++igen) {
      ...
      if (genjet_pt > genPtMin_) {
        ...
        if( doIterativeDeclustering_){
          jets_.genzg[jets_.ngen]  = 0;
          jets_.genrg[jets_.ngen]  = 0;
          jets_.genkt[jets_.ngen]  = 0;
          jets_.genangu[jets_.ngen]= 0;
...
/* Function to fill iterative declustering variables at the reco level */
void HiInclusiveJetAnalyzer::IterativeDeclusteringRec(const reco::Jet& jet,fastjet::PseudoJet *sub1, fastjet::PseudoJet *sub2)
{
  double flagSubjet=0;
  double zg=0;
  double rg=0;
  ...
  try {
    ...
    while(jj.has_parents(j1,j2)){
      ...
      double cut=zcut_*pow(delta_R/rParam, beta_);
      z=j2.perp()/(j1.perp()+j2.perp());

      //standard SD 
      if( (z > cut) && (flagSubjet==0)){
        zg=z;
        rg=delta_R;
        flagSubjet=1;
        j1first =j1;
      ...
      double aparam = 1.0; 
      double dyn=z*(1-z)*j2.perp()*pow(delta_R/rParam,aparam);
  ...
  jets_.jtzg[jets_.nref] = zg;
  jets_.jtrg[jets_.nref]  = rg; 
  jets_.jtkt[jets_.nref]  = kt1;
...
/* Function to fill iterative declustering variables at the ref level */
void HiInclusiveJetAnalyzer::IterativeDeclusteringRef(const reco::GenJet& jet,fastjet::PseudoJet *sub1,fastjet::PseudoJet *sub2){
        double flagSubjet=0;
        double zg=0;
        double nsplit=0;
        ...
        try{
          ...
          jets_.refzg[jets_.nref]  = zg;
          jets_.refrg[jets_.nref]  = rg; 
          jets_.refkt[jets_.nref] = kt1;
          jets_.refangu[jets_.nref]= angu; 
        } catch (fastjet::Error&) { /*return -1;*/ }
}

/* Function to fill iterative declustering variables at the Gen level */
void HiInclusiveJetAnalyzer::IterativeDeclusteringGen(const reco::GenJet& jet,fastjet::PseudoJet *sub1,fastjet::PseudoJet *sub2){
        double flagSubjet=0;
        double zg=0;
        double nsplit=0;
        ...
        try{
          ...
          jets_.genzg[jets_.ngen]  = zg;
          jets_.genrg[jets_.ngen]  = rg; 
          jets_.genkt[jets_.ngen] = kt1;
          jets_.genangu[jets_.ngen]= angu; 
