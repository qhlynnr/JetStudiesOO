...
void RooSimplepTPbPb_Trivial2D(std::string date)
{
  ...
  std::vector<Double_t> kBinsMeasured  = {-0.05, 0, 0.02, 0.04, 0.06, 0.1, 0.2, 0.35}; 
  std::vector<Double_t> kBinsUnfolded = {-0.05, 0, 0.02, 0.04, 0.06, 0.1, 0.2, 0.35, 0.6}; 

 TH2D *h2raw(0);
   h2raw=new TH2D("r","raw",kBinsMeasured.size()-1,kBinsMeasured.data(), 13 ,xbins);
   //detector measure level

   TH2D *h2smeared(0);
   h2smeared=new TH2D("smeared","smeared",kBinsMeasured.size()-1,kBinsMeasured.data(),13,xbins);
   ...
   h2smearednocuts=new TH2D("smearednocuts","smearednocuts",kBinsMeasured.size()-1,kBinsMeasured.data(),13,xbins);
   //true correlations with measured cuts

    TH2D *h2true(0);
    h2true=new TH2D("true","true",kBinsUnfolded.size()-1,kBinsUnfolded.data(),13 ,xbins);
    //full true correlation
    TH2D *h2fulleff(0);
    h2fulleff=new TH2D("truef","truef",kBinsUnfolded.size()-1,kBinsUnfolded.data(),13 ,xbins);
   

    TH2D * h2uf  = new TH2D("h2uf","h2uf",kBinsUnfolded.size()-1,kBinsUnfolded.data(),13 ,xbins);
    TH2D* h2resppT(0);
   h2resppT = new TH2D("h2resppT", "h2resppT", 13 ,xbins, 13 ,xbins);
...
//  effok7->SetName("correff80-120"); 
  ...
  TFile *fout=new TFile (Form("Unfold2D_TrivialTest_%s.root", date.c_str()),"RECREATE");
